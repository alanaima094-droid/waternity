...
import { mockWells, mockTransactions, getWellById } from '@/lib/mock-data';
import { 
  Wallet, 
  Droplets, 
  RefreshCw,
...
const UserBuyWater = () => {
  ...
  const [currentStep, setCurrentStep] = useState<'deposit' | 'liter' | 'refund'>('deposit');
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <div className="text-blue-100 text-sm mt-1">
            ≈ {(userBalance / selectedWell.pricePerLiter).toFixed(1)} liters available
          </div>
        </div>
  ...
  const SessionPanel = () => (
    <div className="bg-[hsl(var(--card))] p-6 rounded-lg border border-[hsl(var(--border))] mb-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold text-[hsl(var(--foreground))]">Current Session</h3>
        <Badge className={sessionData.sessionStart ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div className="text-center p-4 bg-[hsl(var(--muted))] rounded-lg">
          ...
          <div className="text-sm text-[hsl(var(--muted-foreground))]">Dispensed</div>
        </div>
        ...
        <div className="text-center p-4 bg-[hsl(var(--muted))] rounded-lg">
          ...
          <div className="text-sm text-[hsl(var(--muted-foreground))]">Spent</div>
        </div>
        ...
        <div className="text-center p-4 bg-[hsl(var(--muted))] rounded-lg">
          ...
          <div className="text-2xl font-bold text-[hsl(var(--foreground))]">
            {sessionData.sessionStart ? 
              Math.floor((Date.now() - sessionData.sessionStart.getTime()) / 60000) : 0}m
          </div>
  ...
  const DepositFlow = () => (
    <div className="bg-[hsl(var(--card))] p-6 rounded-lg border border-[hsl(var(--border))]">
      ...
      <div className="space-y-4">
        <div>
          ...
          {depositAmount && (
            <div className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
              ≈ {(parseFloat(depositAmount) / selectedWell.pricePerLiter).toFixed(1)} liters
            </div>
          )}
        </div>
        ...
        {sessionData.transactions.filter(tx => tx.type === 'deposit').slice(0, 3).map((tx) => (
          <div key={tx.id} className="flex items-center gap-3 p-3 bg-[hsl(var(--muted))] rounded-lg">
      ...
      <div className="bg-[hsl(var(--card))] p-6 rounded-lg border border-[hsl(var(--border))]">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-[hsl(var(--muted-foreground))] mb-4">
            ...
            <Badge className="ml-2">${selectedWell.pricePerLiter.toFixed(3)}/L</Badge>
          </div>
          ...
          <div>
            ...
            {literAmount && (
              <div className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
                Cost: ${(parseFloat(literAmount) * selectedWell.pricePerLiter).toFixed(2)}
              </div>
            )}
          </div>
          ...
          <Button 
            ...
            disabled={!literAmount || parseFloat(literAmount) <= 0 || (parseFloat(literAmount) * selectedWell.pricePerLiter) > userBalance}
          >
            <Droplets className="h-4 w-4 mr-2" />
            Buy {literAmount || '0'}L for ${literAmount ? (parseFloat(literAmount) * selectedWell.pricePerLiter).toFixed(2) : '0.00'}
          </Button>
          
          {sessionData.transactions.filter(tx => tx.type === 'withdrawal').slice(0, 3).map((tx) => (
            <div key={tx.id} className="flex items-center gap-3 p-3 bg-[hsl(var(--muted))] rounded-lg">
      ...
      <div className="space-y-4">
        <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
          ...
          <div className="text-sm text-yellow-700">
            Refunds are processed within 24-48 hours. A 2% processing fee applies.
          </div>
        </div>
        ...
        <div>
          ...
          {refundAmount && (
            <div className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
              ...
              You'll receive: ${(parseFloat(refundAmount) * 0.98).toFixed(2)}
            </div>
          )}
        </div>
        ...
        <Button 
          ...
          disabled={!refundAmount || parseFloat(refundAmount) <= 0 || parseFloat(refundAmount) > userBalance}
        >
          <RefreshCw className="h-4 w-4 mr-2" />
          Request Refund
        </Button>
        
        {sessionData.transactions.filter(tx => tx.type === 'refund').slice(0, 3).map((tx) => (
          <div key={tx.id} className="flex items-center gap-3 p-3 bg-[hsl(var(--muted))] rounded-lg">
      ...
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div className="bg-[hsl(var(--background))] rounded-lg max-w-2xl w-full max-h-[80vh] overflow-hidden">
          <div className="flex justify-between items-center p-6 border-b border-[hsl(var(--border))]">
            <h3 className="text-lg font-semibold text-[hsl(var(--foreground))]">Transaction Receipts</h3>
            <Button 
          ...
          <div className="p-6 overflow-y-auto max-h-[60vh]">
            <div className="space-y-4">
              {sessionData.transactions.map((tx) => (
                <div key={tx.id} className="bg-[hsl(var(--card))] p-4 rounded-lg border border-[hsl(var(--border))]">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <div className="font-medium text-[hsl(var(--foreground))]">{tx.description}</div>
                      <div className="text-sm text-[hsl(var(--muted-foreground))]">
                        {new Date(tx.timestamp).toLocaleString()}
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="font-bold text-[hsl(var(--foreground))]">${tx.amount.toFixed(2)}</div>
                      <Badge className={tx.status === 'confirmed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}>
                        {tx.status}
                      </Badge>
                    </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <Button size="sm" variant="outline" onClick={() => toast.success('Receipt downloaded (mock)')}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          >
            {mockWells.filter(well => well.status === 'Active').map((well) => (
